...
namespace souz
{
    public partial class FMain : Form
    {
        ...
        private string buildFormSQLQuery(bool whereOnly = false)
            ...
            {
                if (where != null) { where += " AND "; }
                where += @" ( (OPLATA_DO <=  Now()) AND (ITGNW >= DOLG_POSLE_OPLATY)"  + ") ";
            }
            ...
            if ( archiveDataSet.selLS.Count > 0)
                ...
                if ((cOMMENTRichTextBox.Text != "")||(oPLATA_DOTextBox.Text != ""))
                    ...
                    if (MessageBox.Show("Вы подтверждаете, что хотите удалить комментарий?", "Запрос на удаление", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                        string deleteSql = "DELETE FROM COMMENT WHERE COMMENT.LS = "
            ...
            if (archiveDataSet.selLS.Count > 0)
                ...
                if ((cOMMENTRichTextBox.Text != "") || (oPLATA_DOTextBox.Text != ""))
                    ...
                    cmd.CommandText = "UPDATE Comments "
                        + "SET COMMENT = @COMMENT, OPLATA_DO = @OPLATA_DO, DOLG_POSLE_OPLATY = @DOLG_POSLE_OPLATY "
                        + " WHERE (LS = @LS) AND (NZEU = @NZEU)";
...
*/
        ...
        private void doExportToWord()
        {
            ...
            if (archiveDataSet.selLS.Count > 0)
            {
                ...
                cmd.ExecuteNonQuery();
             /*   sql =
                    "INSERT INTO Kvit(LS, ST_FIO, DT_SOST_NA, NUM_SUMMA, DT_OPLATA_DO, NUM_OPLATA_USL, NUM_PEN, NUM_GOS_POSH, NUM_ITOGO) "
                    + " SELECT LS, FIO, NOW(),ITGNW, OPLATA_DO, @NUM_OPLATA_USL, PEN, @NUM_GOS_POSH, ITGNW+@NUM_OPLATA_USL+PEN+@NUM_GOS_POSH  FROM selLS " + buildFormSQLQuery(true) + ""; */
                sql = 
                     "INSERT INTO "
                        + "Kvit( "
                        + "LS, "  
                    ...
                    + "SELECT "
                        ...
                        + " OPLATA_DO, "  
                        + " @NUM_OPLATA_USL, " 
                        + " Round(ITGNW/10, 2) as PEN, " 
                        + " @NUM_GOS_POSH, " 
                        + " ROUND(ITGNW+@NUM_OPLATA_USL+ITGNW/10+@NUM_GOS_POSH,2) as NUM_ITOGO, " 
                        + " Street, " 
                        + " DOML, " 
                        + " DOM, " 
						+ " DOMP, " 
                ...
                {
                    cmd.ExecuteNonQuery();
                    sql = "UPDATE Kvit SET DT_OPLATA_DO = DateAdd(\"ww\";2;Date())";
                    cmd.CommandText = sql;
                    cmd.CommandText = sql;
                }
                catch (Exception e)
                ...
                {
                    if (System.IO.File.Exists(@"C:\souzTemplates\квитанции в суд.dot"))
                    {
                        Process.Start(@"C:\souzTemplates\квитанции в суд.dot");
                    }
                }
            }
            else
